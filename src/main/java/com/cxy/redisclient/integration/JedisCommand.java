...
import com.cxy.redisclient.memcached.MemcachedManager;
import com.cxy.redisclient.memcached.MemcachedManagerImpl;
...
public abstract class JedisCommand implements Comparable<JedisCommand> {
    ...
    protected MemcachedManager memCachedClient;
    ...
    public void execute() {
        ...
        memCachedClient = new MemcachedManagerImpl(server.getHost(), server.getPort());
...
/*        if (server.getPassword() != null && server.getPassword().length() > 0)
    ...
    public void execute(MemcachedManager cache) {
